...
use stride_core::task::{Annotation, Uda};
...
fn init_uda_functions(db: &Connection) -> Result<()> {
    db.create_scalar_function(
        "stride_uda_array_insert",
        2,
        FunctionFlags::SQLITE_UTF8 | FunctionFlags::SQLITE_DETERMINISTIC,
        move |ctx| {
            assert_eq!(ctx.len(), 2, "called with unexpected number of arguments");
            ...
            let mut array = ctx
                ...
                .as_blob_or_null()
                .map_err(|e| rusqlite::Error::UserFunctionError(e.into()))?
                .map(|mut blob| Vec::<Uda>::from_blob(&mut blob))
                .transpose()
                .map_err(|e| rusqlite::Error::UserFunctionError(e.into()))?
                .unwrap_or_default();

            let mut uda_blob = ctx
                .get_raw(1)
                .as_blob()
                .map_err(|e| rusqlite::Error::UserFunctionError(e.into()))?;
            let uda = Uda::from_blob(&mut uda_blob)
                .map_err(|e| rusqlite::Error::UserFunctionError(e.into()))?;
    ...
    db.create_scalar_function(
        "stride_uda_array_remove",
        2,
        FunctionFlags::SQLITE_UTF8 | FunctionFlags::SQLITE_DETERMINISTIC,
        move |ctx: &Context<'_>| -> Result<Option<Cow<'_, [u8]>>, rusqlite::Error> {
            assert_eq!(ctx.len(), 2, "called with unexpected number of arguments");

            let Some(mut uda_array_blob) = ctx
                .get_raw(0)
                .as_blob_or_null()
                .map_err(|e| rusqlite::Error::UserFunctionError(e.into()))?
            else {
            ...
            let mut array = Vec::<Uda>::from_blob(&mut uda_array_blob)
                .map_err(|e| rusqlite::Error::UserFunctionError(e.into()))?;
            ...
            if let Some(index) = array.iter().position(|element| *element == uda) {
                array.remove(index);
            }
