...
pub fn push_operations_diff_task(
    ...
    push_operations_diff_task_with_timestamp(current, previous, Utc::now(), ops);
}

#[allow(clippy::too_many_lines)]
pub fn push_operations_diff_task_with_timestamp(
    current: Option<&Task>,
    previous: Option<&Task>,
    ...
    match (current, previous) {
        ...
        (Some(current), None) => {
            ops.push(OperationKind::TaskCreate { id: current.uuid }.with_timestamp(timestamp));
        }
        (None, Some(_previous)) => {
