...
fn main() -> anyhow::Result<()> {
    ...
    match args.mode {
        ...
        Mode::Add { content } => {
            ...
            let mut task = Task::new(content.trim().to_string());
            task.tags = vec!["Hello".to_string(), "Goodbye".to_string()];
            task.project = Some("aesnatsionast".to_string());
